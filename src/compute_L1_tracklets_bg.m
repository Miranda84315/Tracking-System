function compute_L1_tracklets_bg(opts)
...
for iCam = 1:8
    ...
    load(fullfile(opts.detection,'openpose_bg', sprintf('camera%d.mat',iCam)));
    detections2 = detections;
    load(fullfile(opts.detection,'openpose', sprintf('camera%d.mat',iCam)));
    ...
    for window_start_frame   = start_frame : opts.tracklets.window_width : end_frame
        ...
        detections_in_window_bg = all_dets_bg(window_inds,:);
        ...
        [valid, detections_in_window] = getValidDetections_bg(detections_in_window, detections_in_window_bg, detections_conf, num_visible, opts, iCam);
